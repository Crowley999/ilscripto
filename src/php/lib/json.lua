...
local function grok_string(self, text, start, options)
   --[=[HANDLE JSON_TRY_FIXING]=]    if options.JSON_TRY_FIXING and text:sub(start,start) == '}' then return nil, tostring(start + 1) end
...
local function grok_object(self, text, start, options)
   ...
   while i <= text_len do
      ...
      --[=[HANDLE JSON_TRY_FIXING]=]    if type(new_i) == 'string' then return VALUE, tonumber(new_i) end
      i = skip_whitespace(text, new_i)
      --[==[CHANGED]==]    keyn = tonumber(key); if keyn then key = keyn end
      if text:sub(i, i) ~= ':' then
...
grok_one = function(self, text, start, options)
   ...
   if text:find('^"', start) then
      return grok_string(self, text, start, options)--[==[CHANGED]==]
   ...
   --[=[HANDLE JSON_TRY_FIXING]=]    elseif options.JSON_TRY_FIXING and text:sub(start,start) == ']' then return nil, tostring(start + 1) -- return string so that '{"1":]}' will cause error
   else
